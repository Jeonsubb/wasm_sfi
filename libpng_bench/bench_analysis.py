...
def build_compression_series(rows, bench_name):
    series = {variant: [] for variant in VARIANTS}
    for variant in series:
        for size in SIZES:
            values = []
            for row in rows:
                ...
                if row.get("ret") != "0":
                    continue
                raw = int(row.get("size", "0"))
                if raw <= 0:
                    continue
                ratio_text = row.get("compression_ratio", "")
                if ratio_text not in ("", None):
                    try:
                        values.append(float(ratio_text))
                        continue
                    except ValueError:
                        pass
                # Backward compatibility: older CSV may not have compression_ratio.
                try:
                    encoded_len = float(row.get("encoded_len", ""))
                except ValueError:
                    continue
                values.append(encoded_len / float(raw))
            series[variant].append(median(values) if values else None)
    return series
...
def write_compression_csv(series, out_csv):
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["size", "mem0_ratio", "mem1_ratio", "sfi_ratio"])
        for idx, size in enumerate(SIZES):
            w.writerow([size, series["mem0"][idx], series["mem1"][idx], series["sfi"][idx]])
    print(f"wrote: {out_csv}")


def write_compression_svg(series, out_svg, title):
    width = 900
    height = 540
    margin = 60
    plot_w = width - margin * 2
    ...
    if not all_vals:
        print("no data for compression chart")
        return
    ...
    def y_pos(val):
        if max_y <= min_y:
            return margin + plot_h / 2
        return margin + plot_h * (1.0 - ((val - min_y) / (max_y - min_y)))

    colors = {"mem0": "#1b7f5a", "mem1": "#c65b2b", "sfi": "#2a62c6"}
    lines = []
    lines.append('<?xml version="1.0" encoding="UTF-8"?>')
    lines.append(f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">')
    lines.append('<rect width="100%" height="100%" fill="#ffffff"/>')
    lines.append(f'<rect x="{margin}" y="{margin}" width="{plot_w}" height="{plot_h}" fill="#fafafa" stroke="#d0d0d0"/>')
    lines.append(f'<text x="{margin}" y="{margin - 20}" font-family="sans-serif" font-size="16" fill="#222">{title}</text>')
    lines.append(f'<text x="{margin}" y="{margin - 4}" font-family="sans-serif" font-size="12" fill="#444">compression ratio = encoded_len / raw_size (lower is better)</text>')
    ...
    for idx, name in enumerate(VARIANTS):
        ...
        if points:
            lines.append(f'<polyline fill="none" stroke="{color}" stroke-width="2" points="{" ".join(points)}"/>')
        lines.append(f'<rect x="{legend_x}" y="{legend_y + idx * 18}" width="12" height="12" fill="{color}"/>')
        lines.append(f'<text x="{legend_x + 18}" y="{legend_y + idx * 18 + 10}" font-family="sans-serif" font-size="12" fill="#222">{name}</text>')

    lines.append(f'<text x="{margin}" y="{height - 10}" font-family="sans-serif" font-size="11" fill="#666">x: input size, y: encoded/raw ratio</text>')
    lines.append("</svg>")
    with open(out_svg, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"wrote: {out_svg}")
...
def merge_bench(bench_name, mem0_csv, mem1_csv, out_prefix, title_prefix):
    ...
    compression_series = build_compression_series(all_rows, bench_name)
    ...
    ratio_csv = os.path.join(RESULT_DIR, f"{out_prefix}_compression_ratio.csv")
    write_compression_csv(compression_series, ratio_csv)
    ratio_svg = os.path.join(RESULT_DIR, f"{out_prefix}_compression_ratio.svg")
    write_compression_svg(compression_series, ratio_svg, f"{title_prefix} Compression Ratio")


def main():
    mem0_encode = os.path.join(RESULT_DIR, "mem0_encode.csv")
    mem1_encode = os.path.join(RESULT_DIR, "mem1_encode.csv")
    sfi_encode = os.path.join(RESULT_DIR, "sfi_encode.csv")
    mem0_decode = os.path.join(RESULT_DIR, "mem0_decode.csv")
    mem1_decode = os.path.join(RESULT_DIR, "mem1_decode.csv")
    sfi_decode = os.path.join(RESULT_DIR, "sfi_decode.csv")

    for path in (mem0_encode, mem1_encode, sfi_encode, mem0_decode, mem1_decode, sfi_decode):
