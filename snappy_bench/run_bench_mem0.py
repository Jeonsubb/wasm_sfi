...
VARIANT = "mem0"
WASM = os.path.join(ROOT, "build", "mem0", "final_mem0.wasm")
...
def run_bench(fresh_funcs, out_csv_compress, out_csv_uncompress):
    comp_rows = []
    uncomp_rows = []
    for size in SIZES:
        for i in range(RUNS):
            store, prepare_func, compress_func, uncompress_func, get_compressed_len_func, verify_uncompress_func = fresh_funcs()
            try:
...
def main():
    ...
    run_bench(
        fresh_funcs,
        os.path.join(RESULT_DIR, "mem0_compress.csv"),
        os.path.join(RESULT_DIR, "mem0_uncompress.csv"),
    )
